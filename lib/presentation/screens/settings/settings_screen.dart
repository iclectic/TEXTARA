...
class SettingsScreen extends ConsumerWidget {
  ...
  Future<void> _importBackup(BuildContext context, WidgetRef ref) async {
    ...
    try {
      ...
      final exportService = ref.read(exportServiceProvider);
      final importResult = await exportService.importBackup(filePath);

      if (!context.mounted) return;

      if (importResult.success && importResult.backup != null) {
        ref.read(booksProvider.notifier).loadBooks();
        final backup = importResult.backup!;
        final message = importResult.isMetadataOnly
            ? 'Restored metadata for ${backup.books.length} books. ${importResult.missingBookFiles} files need to be re-imported on this device.'
            : 'Restored ${backup.books.length} books, ${backup.highlights.length} highlights.';
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(message)));
      } else {
        final message = importResult.validationIssues.isNotEmpty
            ? importResult.validationIssues.first
